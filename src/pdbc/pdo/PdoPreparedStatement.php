...
use dev\winterframework\pdbc\util\StatementType;
use PDO;
use PDOStatement;
...
class PdoPreparedStatement extends AbstractPreparedStatement {
    ...
    public function execute(): bool {
        $this->reset();
        if ($this->doExecute()) {
            $this->loadGeneratedKeysIfInsert();
            $this->resultSet = new PdoResultSet($this, $this->cursorName);
            return true;
        }
        return false;
    ...
    /**
     * Retrieve generated keys only for INSERT statements on drivers that support
     * PDO::lastInsertId() as their native generated-key mechanism.
     *
     * This deliberately skips UPDATE/DELETE/SELECT and other statements so that,
     * for example, PostgreSQL's lastval()/currval() is never invoked for a
     * statement that did not generate a sequence value.
     *
     * PostgreSQL INSERT ... RETURNING is handled through the normal result-set
     * path (executeQuery()/queryForObject()), not through this method.
     */
    private function loadGeneratedKeysIfInsert(): void {
        if (!$this->connection->supportsLastInsertId()) {
            return;
        }
        if (!StatementType::isInsert($this->sql)) {
            return;
        }
