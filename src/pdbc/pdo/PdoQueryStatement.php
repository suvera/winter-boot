...
class PdoQueryStatement extends AbstractStatement {
    ...
    public function execute(
        ...
        if ($autoGeneratedKeys == self::RETURN_GENERATED_KEYS) {
            $this->loadGeneratedKeysIfInsert($sql, $pdo, $columnIdxOrNames);
        }
        $this->resultSet = new PdoResultSet($this, $this->cursorName);
        return true;
    }
    ...
    /**
     * Retrieve generated keys only when:
     *  - RETURN_GENERATED_KEYS was explicitly requested, AND
     *  - the driver natively supports PDO::lastInsertId(), AND
     *  - the statement is an INSERT.
     *
     * This prevents UPDATE/DELETE/SELECT from triggering PostgreSQL's
     * lastval()/currval() and other non-portable driver behavior.
     */
    private function loadGeneratedKeysIfInsert(
        string $sql,
        PDO $pdo,
        array $columnIdxOrNames
    ): void {
