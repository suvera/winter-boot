...
class EnableAsync implements StereoType {
    ...
    public function init(object $ref): void {
        ...
        if (empty($app)) {
            throw new TypeError("Class Annotated with #[EnableAsync] "
                . "must have also annotated with #[WinterBootApplication] for class "
                . ReflectionUtil::getFqName($ref));
        }

        if (!extension_loaded('swoole')) {
            throw new AnnotationException("Annotation #[EnableAsync] requires *swoole* extension in PHP runtime "
                . ReflectionUtil::getFqName($ref));
        }
    }
