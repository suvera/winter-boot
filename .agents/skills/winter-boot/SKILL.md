...
- `HttpRequest` is injectable as a handler-method param. Read everything from it: `getMethod/getUri/getQueryParam(s)/getPostParam(s)/getFirstHeader/getCookie(s)/getRawBody/getContentType/getFile(s)`, plus caller/server getters `getRemoteAddr/getRemotePort/getServerAddr/getServerPort/getServerProtocol/getQueryString/getRequestTime/getRequestTimeFloat` (nullable). NEVER read `$_SERVER`/`$_COOKIE` under Swoole (shared worker process — values belong to another request); `SwooleRequest` does not mirror them.
...
## 8. Request Sessions (`dev\winterframework\web\session`)

Coroutine-safe sessions: no superglobals, no `session_*` anywhere in this namespace. `SessionManager` (stateless, autowired bean) has two ops: `open(HttpRequest $req, SessionHandlerInterface $store, SessionOptions $o): RequestSession` (id from `$req->getCookie($o->name)` only; bad/missing id → fresh id via `random_bytes`, marked `isNew()`), `commit(RequestSession $s, ResponseEntity $res, ...)` (writes + emits the session cookie; `destroy()` + commit deletes the row + emits expired cookie). `RequestSession` bag: `getId/isNew/get/set/remove/destroy` + identity `getUsername/setUsername` (default `''`), `getSessionType/setSessionType` (default `0`, app-defined meaning); `open()` re-hydrates identity so recommit never wipes it.
- `SessionOptions` is NOT a bean: `new SessionOptions(name:, expirySecs: (0 = browser-session), path:'/', domain:'', secure:false, httponly:true)` per login flow (e.g. `ADMINSESSID` 900s vs `USERSESSID` 86400s); different names = independent sessions sharing one store.
- Defaults (no SessionConfig needed): `SessionManager` + file-based `\SessionHandler` beans, overwrite-safe. Override the store with a `#[Bean]` returning `\SessionHandlerInterface`.
- `PdbcSessionStore(PdbcTemplate $db, table:'winter_sessions', ttlSecs:0)` implements `SessionIdentityStore` (`readWithIdentity/writeWithIdentity`; plain `write()` unattributed). DDL: `session_id VARCHAR(128) NOT NULL PRIMARY KEY, username VARCHAR(255) NOT NULL, expiry/created_at/updated_at BIGINT DEFAULT 0, session_type SMALLINT DEFAULT 0, session_data TEXT`. Payload is base64'd `serialize()`; write = UPDATE→INSERT→retry (no vendor upsert).
- Redis: `dev\winterframework\data\redis\session\RedisSessionStore` in winter-data-redis takes `PhpRedisTemplate` (`keyPrefix:'wbsess:', ttlSecs:3600`, native TTL).
- Tests: `srcTests/RequestSessionTest.php`, `SessionStoreTest.php`, `SwooleRequestTest.php` (namespace `winterBootTests`, extend `Support\TestCase`); run `php srcTests/run.php` from repo root.
